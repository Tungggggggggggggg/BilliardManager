...
export const getAllInvoices = async () => {
  try {
    const result = await pool.query('SELECT * FROM Invoice ORDER BY invoice_id');
    return result.rows;
  } catch (error) {
...
export const getInvoiceById = async (id) => {
  try {
    const result = await pool.query('SELECT * FROM Invoice WHERE invoice_id = $1', [id]);
...
export const updateInvoice = async (id, { customer, phone, date, total, status, member_id }) => {
  try {
    const result = await pool.query(
      'UPDATE Invoice SET customer = $1, phone = $2, date = $3, total = $4, status = $5, member_id = $6 WHERE invoice_id = $7 RETURNING *',
...
export const getInvoiceItems = async (invoiceId) => {
  try {
    const result = await pool.query('SELECT * FROM InvoiceItem WHERE invoice_id = $1', [invoiceId]);
    return result.rows;
  } catch (error) {
...
export const createInvoiceItem = async (invoiceId, { inventory_item_id, name, quantity, price }) => {
  try {
    const result = await pool.query(
      'INSERT INTO InvoiceItem (invoice_id, inventory_item_id, name, quantity, price) VALUES ($1, $2, $3, $4, $5) RETURNING *',
